...
st.dataframe(total_race_df[(total_race_df['Measure'].isin(race_filter)) & (total_race_df['FIVE_YEAR_ESTIMATE_DATE'] == total_race_df['FIVE_YEAR_ESTIMATE_DATE'].max())][['Measure', 'Counts']].sort_values(by=['Counts'], ascending = False).set_index(['Measure']),
            use_container_width = True)
